...
csv_dir_spmm = csv_dir + "DenseAblationKProfileSuite/bsr_spmm"
...
with open(json_output_dir + "data_dicts_overhead_analysis.json", "w") as f:
...
ax.set_title('Sparse Indexing Overhead Comparison')
ax.set_xticks(x + bar_width)
ax.set_xticklabels([f'K={k}' for k in k_vals], rotation=45, ha='right')
ax.legend(['Block SpMM', 'Dense Matmul Basic', 'Dense Matmul Multicast'])

plt.tight_layout()
plt.show()

plt.savefig(png_output_dir + "program_loop_total_ns_overhead_analysis.png")
