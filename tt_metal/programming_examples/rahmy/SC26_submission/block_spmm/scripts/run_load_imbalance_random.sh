...
###############################################################################
# Load-Imbalance Experiment (Random 25% Density)
#
# Runs random 25% density test case (registry 4 / PatternD25, test 3) on:
#   hc 0  Naive (with LB)     hc 16 Naive (no LB)
#   hc 1  SnF (with LB)       hc 17 SnF (no LB)
#   hc 2  CDA (with LB)       hc 15 CDA (no LB)
#
# Test 3: random, M=N=K=8192, R=C=256, d=25%
#
...
# ── Defaults ─────────────────────────────────────────────────────────────────
OUTPUT_DIR="profiles_load_imbalance_random"
DRY_RUN=0
EXPORT_ONLY=0
PROFILE_ONLY=0
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  --output-dir DIR  Output folder name under repo root (default: profiles_load_imbalance_random)"
...
# ── Main ─────────────────────────────────────────────────────────────────────
echo "Load-Imbalance Experiment: Random 25% (reg $REGISTRY, test 3) x 6 host codes"
