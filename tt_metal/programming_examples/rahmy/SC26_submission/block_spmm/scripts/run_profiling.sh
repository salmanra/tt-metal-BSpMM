...
#
# Orchestrates four experiment phases:
...
#   lowdensity  - Ultra-low density    (registries 10-11, host codes 0-2)
###############################################################################
...
# ── Registry metadata (indices 0-11, aligned with C++ Registries[] array) ────
REGISTRY_NAMES=(
    "MicrobenchD25"       # 0
    "MicrobenchD5"        # 1
    "PatternD5"           # 2
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  --phase microbench|throughput|scaling|lowdensity|all   Experiment phase (default: all)"
...
case "$PHASE" in
    ...
    *) echo "Error: invalid phase '$PHASE'. Must be microbench|throughput|scaling|lowdensity|all" >&2; exit 1 ;;
esac
...
if [[ $LIST_ONLY -eq 1 ]]; then
    ...
    echo "  lowdensity  : registries 10-11,  host codes 0-2   => 2 * 6 * 3  =  36 runs"
...
run_lowdensity() {
    echo ""
    echo "================================================================"
    echo "  Phase: lowdensity (Experiment 4 - Ultra-Low Density)"
