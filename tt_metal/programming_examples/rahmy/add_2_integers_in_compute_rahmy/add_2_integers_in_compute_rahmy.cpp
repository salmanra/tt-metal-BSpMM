...
int main(int argc, char** argv) {
    ...
    KernelHandle binary_reader_kernel_id = CreateKernel(
        program,
        "tt_metal/programming_examples/rahmy/add_2_integers_in_compute_rahmy/kernels/dataflow/reader_binary_1_tile.cpp",
        core,
        DataMovementConfig{.processor = DataMovementProcessor::RISCV_1, .noc = NOC::RISCV_1_default});

    KernelHandle unary_writer_kernel_id = CreateKernel(
        program,
        "tt_metal/programming_examples/rahmy/add_2_integers_in_compute_rahmy/kernels/dataflow/writer_1_tile.cpp",
        core,
    ...
    KernelHandle eltwise_binary_kernel_id = CreateKernel(
        program,
        "tt_metal/programming_examples/rahmy/add_2_integers_in_compute_rahmy/kernels/compute/add_2_tiles.cpp",
        core,
        ComputeConfig{
    ...
    std::pair<bfloat16, bfloat16> unpacked = unpack_two_bfloat16_from_uint32(result_vec[0]);

    float first_value = unpacked.first.to_float();
    float second_value = unpacked.second.to_float();
