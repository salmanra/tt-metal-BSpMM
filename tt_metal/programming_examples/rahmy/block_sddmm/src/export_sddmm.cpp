...
void export_to_csv(int host_code_num, int test_num, ProfileCaseFunctionPtr *Registry, std::string registry_name, SDDMMHostCodeRegistryType* hc_registry){
    ...
    char buf[1000];
    size_t n = sprintf(buf, "/home/user/tt-metal/sddmm_profiles/opt/traces/%s/%s/", registry_name.c_str(), host_function_name.c_str());
    std::string trace_directory(buf, n);
    std::string trace_file_location = trace_directory + test_file_name + ".tracy";

    n = sprintf(buf, "/home/user/tt-metal/sddmm_profiles/opt/csvs/%s/%s/", registry_name.c_str(), host_function_name.c_str());
