...
uint32_t get_Npc_from_BSR_block_size(uint32_t Nt, uint32_t Mpc, uint32_t in0_block_w, uint32_t num_cores_x, uint32_t num_tiles_for_NoC_args) {
    ...
    auto Npc_max = _get_maximum_block_dim_with_NoC_args(Mpc, in0_block_w, num_tiles_for_NoC_args);
...
void bsr_spmm_multicore_reuse_iteration(
    ...
    // for testing :P
    CoreCoord compute_with_storage_grid_size = {2, 2};
    ...
    */
    ...
    if (verbose){
        ...
        for (size_t i = 0; i < reader_compile_time_args.size(); ++i) {
            log_info(tt::LogVerif, "comptime_reader_arg[{}/{}] ({}) = {}", i, reader_compile_time_args.size(), reader_arg_names[i], reader_compile_time_args[i]);
        }
    }
    ...
    for (int work_region = 0; work_region < num_output_work_regions_total; work_region++){
        ...
        // std::cout << "num folded indices " << folded_bsr_matrix_indices.size() << std::endl;
        for (uint32_t i = 0; i < num_out_blocks_per_core; i++){
            ...
            // // std::cout << "Print " << i << "/" << num_out_blocks_per_core << " times" << std::endl;
            // std::cout << "folded indices [start + i]" << folded_bsr_matrix_indices[folded_output_idx_y_start + i] << std::endl;
            // std::cout << "output_idx_y " << mm_reader_args[i+2] << std::endl;
            ...
            // std::cout << "      Matching print" << i << "/" << num_out_blocks_per_core << " times" << std::endl;
        }
...
void bsr_spmm_multicore_reuse_merge_blocks(
    ...
    // per_core_M = std::min(Rt * (per_core_M / Rt), Mt);
    ...
    if (verbose) {
        ...
        log_info(
            tt::LogVerif,
            " -- Mt={} -- Nt={} -- per_core_M={} -- per_block_M= {} -- per_core_N= {} -- out_subblock_h= {} -- out_subblock_w= {} --",
            Mt,
            Nt,
            per_core_M,
            per_block_M,
    ...
    uint32_t nnz_output_blocks_total = num_blocks_x * nnz_rows; // blocks per row * nnz rows
    uint32_t num_output_work_regions_total = nnz_output_blocks_total / (per_core_M / per_block_M); // multiple blocks per core, so we reflect that
    ...
    */
    ...
    auto reader_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/rahmy/block_spmm/kernels/dataflow/reader_merge_blocks.cpp",
        all_cores,
        tt_metal::DataMovementConfig{
            .processor = DataMovementProcessor::RISCV_0,
            .noc = NOC::RISCV_0_default,
    ...
    auto writer_id = tt_metal::CreateKernel(
        program,
        "tt_metal/programming_examples/rahmy/block_spmm/kernels/dataflow/writer_merge_blocks.cpp",
        all_cores,
        tt_metal::DataMovementConfig{
            .processor = DataMovementProcessor::RISCV_1,
            .noc = NOC::RISCV_1_default,
    ...
    if (verbose){
        CoreRange cores_box = all_cores.bounding_box();
        for (size_t core_x = cores_box.start_coord.x; core_x <= cores_box.end_coord.x; core_x++){
            for (size_t core_y = cores_box.start_coord.y; core_y <= cores_box.end_coord.y; core_y++){
                log_info(tt::LogVerif, "Core ({}, {})", core_x, core_y);
            }
        }
        log_info(tt::LogVerif, "CoreRange: {}", cores_box);
    }

    // for each work region
    //      get the x and y output coordinates
    //      use them?
    // out_idx_y =
